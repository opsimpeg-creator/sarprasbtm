...
function onOpen() {
  ...
  ui.createMenu('🚀 SMKN 1 Batumandi')
    .addItem('1. Inisialisasi Database Otomatis (ROOMS, SARPRAS & ADMINS)', 'setupDatabase')
    .addItem('2. Isi Data Awal Sarpras & User Admin SMKN 1 Batumandi', 'seedDataDefault')
    .addSeparator()
...
/**
 * Membuat Sheet 'ROOMS', 'SARPRAS', dan 'ADMINS' beserta Header Kolom
 */
function setupDatabase() {
  ...
  var headersSarpras = [
    'id', 'room_id', 'nama_barang', 'kategori', 'merk', 'spesifikasi', 
    'kondisi', 'jumlah', 'satuan', 'tahun_pengadaan', 'sumber_dana', 
    'keterangan', 'foto', 'created_at', 'updated_at'
  ];
  sheetSarpras.getRange(1, 1, 1, headersSarpras.length).setValues([headersSarpras]);
  formatHeaderRow(sheetSarpras, headersSarpras.length, '#065f46'); // Emerald Green
...
/**
 * Mengisi Data Sampel Awal Ruangan, Sarpras, dan Admin User
 */
function seedDataDefault() {
  ...
  if (sheetAdmins.getLastRow() === 1) {
    // Default Password Hash (SHA-256 for password 'admin123')
    ...
    sheetAdmins.getRange(2, 1, sampleAdmins.length, sampleAdmins[0].length).setValues(sampleAdmins);
  }

  autoResizeColumns(sheetRooms, 10);
  autoResizeColumns(sheetSarpras, 15);
  autoResizeColumns(sheetAdmins, 9);

  SpreadsheetApp.getUi().alert('🎉 Berhasil mengisi data awal SMKN 1 Batumandi (ROOMS, SARPRAS, & ADMINS dengan Hashed Passwords)!');
}
